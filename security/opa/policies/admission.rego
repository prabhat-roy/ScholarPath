...
deny[msg] if {
    ...
    not startswith(container.image, "harbor.scholarpath.internal/")
    msg := sprintf("image %q must be from harbor.scholarpath.internal", [container.image])
}
