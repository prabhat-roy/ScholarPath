# ScholarPath â€” top-level Makefile
...
help:
	@echo "ScholarPath â€” common targets"
	@echo ""
...
tools-deploy:  ## Deploy every OSS tool registered for ScholarPath via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get scholarpath-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=scholarpath-tools" --grpc-web
