resource "google_compute_global_address" "pg" {
  name          = "scholarpath-${var.environment}-pg-psa"
  purpose       = "VPC_PEERING"
...
resource "google_sql_user" "scholarpath" {
  name     = "scholarpath"
  instance = google_sql_database_instance.postgres.name
  password = random_password.pg.result
}

resource "google_sql_database" "scholarpath" {
  name     = "scholarpath"
  instance = google_sql_database_instance.postgres.name
}
...
# Pub/Sub stands in for Kafka on GCP for cross-domain learning events.
resource "google_pubsub_topic" "xapi" {
  name = "scholarpath-${var.environment}-xapi-statements"
}
