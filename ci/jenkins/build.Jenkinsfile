pipeline {
    ...
    stages {
        ...
        stage('Sign') { steps { sh 'cosign sign --yes ${REGISTRY}/scholarpath/${SERVICE}:${GIT_COMMIT}' } }
        stage('Push') { steps { sh 'make push' } }
    }
    post {
        failure { slackSend channel: '#scholarpath-ci', message: "Build failed: ${env.BUILD_URL}" }
    }
}
